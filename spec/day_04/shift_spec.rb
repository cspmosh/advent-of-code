...
describe Shift do
  describe "#covered_by?" do
    ...
    it "returns true if the shift is fully covered by the other" do
      expect(Shift.new("5-7").covered_by?(first_shift)).to be true
      expect(Shift.new("8-8").covered_by?(first_shift)).to be true
    end

    it "returns false if the shift is not fully covered by the other" do
      expect(Shift.new("1-4").covered_by?(first_shift)).to be false
      expect(Shift.new("7-10").covered_by?(first_shift)).to be false
      expect(Shift.new("20-30").covered_by?(first_shift)).to be false
      expect(Shift.new("1-10").covered_by?(first_shift)).to be false
    end
  end
